  ...
  return createPortal(
    <>
      ...
      >
        {/* Modal Content Card */}
        <div
          className="relative w-full max-w-2xl bg-white dark:bg-ink-900 rounded-3xl shadow-2xl border border-cream-200 dark:border-ink-800 overflow-hidden my-auto cursor-default animate-scale-up"
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-cream-50/80 dark:bg-ink-800 border border-cream-200 dark:border-ink-700 rounded-xl text-xs text-ink-900 dark:text-cream-50 focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 focus:bg-white dark:focus:bg-ink-800 transition-all font-medium"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-cream-50/80 dark:bg-ink-800 border border-cream-200 dark:border-ink-700 rounded-xl text-xs text-ink-900 dark:text-cream-50 focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 focus:bg-white dark:focus:bg-ink-800 transition-all font-medium cursor-pointer"
                >
                  {POPULAR_PROVINCES.map((prov) => (
                    <option key={prov} value={prov} className="dark:bg-ink-900 dark:text-cream-50">
                      {prov}
                    </option>
                  ))}
                </select>
              ...
              <div>
                <label className="block text-xs font-bold text-ink-700 dark:text-cream-200 mb-1">Quận / Huyện</label>
                <input
                  type="text"
                  value={district}
            ...
            <div className="flex items-center gap-2 pt-0.5">
              <input
                ...
                className="w-4 h-4 text-accent-500 rounded border-cream-300 dark:border-ink-700 focus:ring-accent-500 cursor-pointer"
              />
              <label htmlFor="isDefaultAddressCheckbox" className="text-xs font-semibold text-ink-700 dark:text-cream-200 cursor-pointer">
                Đặt làm địa chỉ giao hàng mặc định
              </label>
            </div>

            {/* Actions */}
            <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-cream-100 dark:border-ink-800">
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2 rounded-full text-xs font-bold text-ink-600 dark:text-cream-300 hover:bg-cream-100 dark:hover:bg-ink-800 transition-colors cursor-pointer"
              >
                Hủy Bỏ
              </button>
              <button
